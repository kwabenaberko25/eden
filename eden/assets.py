...
def eden_scripts() -> Markup:
    ...
    return Markup(
        ...
        "          const fullEvent = `${data.event}:${data.channel}`;\n"
        "          \n"
        "          // Dispatch global event (e.g. \"updated:tasks:5\")\n"
        "          document.dispatchEvent(new CustomEvent(fullEvent, { detail: data.data }));\n"
        "          \n"
        "          // Trigger on elements interested in this exact channel or the table base\n"
        "          const baseChannel = data.channel.split(':')[0];\n"
        "          const targets = document.querySelectorAll(`[hx-sync=\"${data.channel}\"], [hx-sync=\"${baseChannel}\"]`);\n"
        "          \n"
        "          targets.forEach(el => {\n"
        "            htmx.trigger(el, fullEvent, data.data);\n"
        "          });\n"
        "        }\n"
        "      };\n"
        "      \n"
